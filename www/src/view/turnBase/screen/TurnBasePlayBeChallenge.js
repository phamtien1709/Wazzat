...
export default class TurnBasePlayBeChallenge extends BaseGroup {
    constructor() {
        super(game);
    ...
    displayAnswering() {
        ...
        // LogConsole.log('SONGPLAY' + this.questions[this.countQuiz].songEntity.fileUrl);
        ...
        this.timeTweenBegin = game.time.events.add(Phaser.Timer.SECOND * 0.5, () => {
            ...
            if (this.countQuiz == 0) {
                ...
                popupTut.afterInit();
                popupTut.addHelperStep4();
                popupTut.changeBg();
                this.addChild(popupTut);
                if (MainData.instance().playScript.playing_guide == PlayScriptScreen.DONE_GET_QUEST) {
                    this.timePS4 = game.time.events.add(Phaser.Timer.SECOND * 1.4, () => {
                        popupTut.addStep4UseSupportItem();
                        popupTut.event.step4.add(this.stepRemoveAnswer, this);
                    });
                } else {
                    popupTut.destroy();
                }
    ...
    addBtnRemoveAnswer() {
        ...
        this.btn_remove_answer.events.onInputUp.add(this.sendRemoveAnswer, this);
    ...
    addAvaMiniInTheLeftOfTab(sprite) {
        ...
        if (!window.MQ.isBotMode) {
            if (this.opponent.avatar !== "") {
                ...
                var ava_friend_challenge = game.add.sprite(-246 * window.GameConfig.RESIZE, 0, 'songDetailSprites', 'ava-default');
                ava_friend_challenge.anchor.set(0.5);
                ava_friend_challenge.scale.set(window.GameConfig.SCALE_AVA_OPPO_INTHELEFT * window.GameConfig.RESIZE);
                ava_friend_challenge.mask = maskAvaFriend;
            }
        ...
        this.timeAvaMiniShow.add(Phaser.Timer.SECOND * (this.opponentAnswerLogs[this.countQuiz].answer_time + 0.4), () => {
    ...
    calculateScoreOpp() {
        if (this.countQuiz <= 4) {
            if (this.opponentAnswerLogs[this.countQuiz].result.toUpperCase() == "CORRECT") {
                this.setTrueAvaOpp();
                var scoreOpp = Math.floor((11 - this.opponentAnswerLogs[this.countQuiz].answer_time) * 100 * (Math.pow(11 - this.opponentAnswerLogs[this.countQuiz].answer_time, ((this.opponentAnswerLogs[this.countQuiz].streak - 1) / 4))));
    ...
    pushResultAnswer() {
        ...
        if (this.timeCounter.ms / 1000 > 10) {
            ...
            let ms = (this.timeCounter.ms / 1000).toFixed(1);
            this.answers.push({
                answer_time: parseFloat(ms),
                result: trueFalse,
                answer: this.answered + 1,
                streak: this.streak,
                question_id: this.questions[this.countQuiz].id,
    ...
    destroy() {
        ...
        game.time.events.remove(this.timePS4);
        game.time.events.remove(this.timeNextQuestion);
