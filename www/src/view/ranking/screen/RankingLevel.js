...
import GetRankingDiamond from "../../../model/ranking/server/getdata/GetRankingDiamond.js";
import RankingItemTopDiamond from "../item/RankingItemTopDiamond.js";
import Ranking from "../Ranking.js";
import SpriteBase from "../../component/SpriteBase.js";
import UserRankingDiamond from "../../../model/ranking/data/UserRankingDiamond.js";
...
export default class RankingLevel extends BaseView {
    constructor() {
        super(game, null);
        ...
        FaceBookCheckingTools.instance().logEvent(FaceBookCheckingTools.Ranking_level);
    ...
    sendGetDataServer() {
        if (this.idxItem < 100) {
            ...
            SocketController.instance().sendData(RankingCommand.RANKING_LEVEL_LOAD_REQUEST,
    ...
    getData(data) {
        switch (data.cmd) {
            case RankingCommand.RANKING_LEVEL_LOAD_RESPONSE:
                let dataRanking = GetRankingDiamond.begin(data.params);
    ...
    buildListRanking() {
        ...
        if (this.positionMe > -1) {
            this.txtError.visible = false;
            if (this.rankMe === null) {
                let data = new UserRankingDiamond();
                ...
                data.diamond = SocketController.instance().dataMySeft.diamond;
                data.level = SocketController.instance().dataMySeft.level;
                data.vip = SocketController.instance().dataMySeft.vip;

                this.rankMe = new RankingItemTopDiamond(data, this.positionMe - 1, Ranking.TOP_LEVEL, true);
                this.rankMe.x = 0;
    ...
    buildItemPage() {
        ...
        for (let i = this.idx; i < this.arrRanking.length; i++) {
            let item = new RankingItemTopDiamond(this.arrRanking[i], i, Ranking.TOP_LEVEL);
