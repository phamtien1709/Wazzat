...
export default class RankingDiamond extends BaseView {
    constructor() {
        super(game, null);
        ...
        FaceBookCheckingTools.instance().logEvent(FaceBookCheckingTools.Ranking_diamond);
    ...
    sendGetDataServer() {
        if (this.idxItem < 100) {
            ...
            SocketController.instance().sendData(RankingCommand.RANKING_PLAYLIST_COLLECTION_REQUEST,
    ...
    getData(data) {
        switch (data.cmd) {
            case RankingCommand.RANKING_PLAYLIST_COLLECTION_RESPONSE:
                ...
                if (this.positionMe === -1) {
                    ...
                    this.playlist_number = dataRanking.position.playlist_number;
                }
    ...
    buildListRanking() {
        if (this.positionMe > -1) {
            ...
            if (this.rankMe === null) {
                ...
                data.playlist_number = this.playlist_number;

                this.rankMe = new RankingItemTopDiamond(data, this.positionMe - 1, Ranking.TOP_DIAMOND, true);
                this.rankMe.x = 0;
    ...
    buildItemPage() {
        ...
        for (let i = this.idx; i < this.arrRanking.length; i++) {
            let item = new RankingItemTopDiamond(this.arrRanking[i], i, Ranking.TOP_DIAMOND);
