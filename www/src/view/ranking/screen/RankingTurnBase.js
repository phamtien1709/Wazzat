...
export default class RankingTurnBase extends BaseView {
    constructor() {
        super(game, null);
        ...
        FaceBookCheckingTools.instance().logEvent(FaceBookCheckingTools.Ranking_turnbase);
    ...
    sendGetDataServer() {
        if (this.idxItem < 100) {
            ...
            SocketController.instance().sendData(RankingCommand.RANKING_TURNBASE_LOAD_REQUEST,
    ...
    getData(data) {
        switch (data.cmd) {
            case RankingCommand.RANKING_TURNBASE_LOAD_RESPONSE:
    ...
    buildListRanking() {
        if (this.positionMe > -1) {
            ...
            if (this.rankMe === null) {
                ...
                this.rankMe = new RankingItemTopDiamond(data, this.positionMe - 1, Ranking.TOP_TURNBASE, true);
                this.rankMe.x = 0;
    ...
    buildItemPage() {
        ...
        for (let i = this.idx; i < this.arrRanking.length; i++) {
            let item = new RankingItemTopDiamond(this.arrRanking[i], i, Ranking.TOP_TURNBASE);
