...
export default class BasicSwiper {
  ...
  handleDown(target, pointer) {
    ...
    // LogConsole.log('handle down', pointer[this.o.direction])
    this.isDown = true;
  ...
  tweenTo(duration, target) {
    // LogConsole.log('tweenTo', duration, target)
  ...
  handleUpdate() {
    this.events.onUpdate.dispatch(
      MathUtils.scaleBetween(-1,
        1,
        MathUtils.percentageBetween2(
          this.scrollObject[this.o.direction], -this.length,
          this.length
        )
      )
    );
