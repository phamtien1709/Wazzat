...
export default class ScrollerEventDispatcher {
  ...
  handleMove(pointer, x, y) {
    ...
    if (!this.direction && this.o.direction == 'auto') {
      ...
      if (
        xDist > this.o.autoDetectThreshold ||
        yDist > this.o.autoDetectThreshold
      ) {
  ...
  _cancelCurrentDown(pointer) {
    if (
      this.currentDown &&
      this.currentDown.events &&
      this.currentDown.events.onInputUp
    ) {
      this.currentDown.events.onInputUp.dispatch(
        this.currentDown,
        pointer,
        false
      );
