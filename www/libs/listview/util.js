...
export function getWidthOrHeight(displayObject, widthOrHeight) {
  ...
  //return PIXI.DisplayObjectContainer.prototype.getBounds.call(displayObject)[widthOrHeight];
}
...
export function dispatchClicks(pointer, clickables, type) {
  ...
  const found = findChild(clickables, clickable => {
    ...
    //LogConsole.log('does ' + x + "  y " + y + " clickable.width " + clickable.width + " clickable.height " + clickable.height + ' intersect x ' + pointer.x + " y " + pointer.y)
    return (
