...
export default class Scroller {
  ...
  handleDown(target, pointer) {
    ...
    this.isDown = true;
    // LogConsole.log('input down', pointer.y)
    ...
    if (
      ...
      // LogConsole.log('swipe while animation is happening', this.o.multiplier)
    } else {
      //reset
      this.o.multiplier = 1;
    }
  ...
  tweenTo(duration, target) {
    ...
    this.tweenScroll = this.game.add
      ...
      .to({
        [this.o.direction]: target
      }, duration * 1000, Phaser.Easing.Quartic.Out);
    this.tweenScroll.onUpdateCallback(this.handleUpdate, this);
    this.tweenScroll.onComplete.add(this.handleComplete, this);
  ...
  handleUpdate() {
    //LogConsole.log("this.enabled : " + this.enabled);
    if (!this.enabled) return;
    if (this.o.infinite) {
    ...
    //LogConsole.log("this.o " + JSON.stringify(this.o));
    ...
    //LogConsole.log("this.dispatchValues : " + JSON.stringify(this.dispatchValues));
